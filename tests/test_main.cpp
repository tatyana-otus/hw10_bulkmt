...
BOOST_AUTO_TEST_CASE(N_input)
{
    ...
    BOOST_CHECK_THROW(process("0"),   std::invalid_argument);
    BOOST_CHECK_THROW(process("0x10"),std::invalid_argument);
    BOOST_CHECK_THROW(process("a34"), std::invalid_argument);

    auto str_1 = std::to_string(MAX_BULK_SIZE + 1);
    BOOST_CHECK_THROW(test_process(str_1.c_str(), "", ""), std::invalid_argument);

    auto str_2 = std::to_string(MAX_BULK_SIZE);
    BOOST_CHECK_NO_THROW(test_process(str_2.c_str(), "", "", false));
}
...
BOOST_AUTO_TEST_CASE(max_command_length)
{
    ...
    BOOST_CHECK_NO_THROW(test_process("1", in_data_1, out_data_1, false));

    std::string in_data_2 (MAX_CMD_LENGTH + 1, 'a');    
    BOOST_CHECK_THROW(test_process("1", in_data_2, ""), std::invalid_argument);
}
